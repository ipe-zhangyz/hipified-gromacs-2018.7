...
/* XXX switch between chevron and hipLaunch (supported only in CUDA >=7.0)
                                                       ...
                                                       const gmx_device_info_t gmx_unused  *devInfo)
{
    ...
    GMX_ASSERT(c_nbnxnGpuClusterSize*c_nbnxnGpuClusterSize/c_nbnxnGpuClusterpairSplit == devInfo->prop.hipWarpSize,
                             ...
                             int                     iloc)
{
    ...
    if (bUseCudaLaunchKernel)
    {
        ...
        hipLaunchKernel((void *)nb_kernel, dim_grid, dim_block, kernel_args, shmem, stream);
#endif
    }
    else
    {
                                       ...
                                       int                     numParts)
{
    ...
    if (bUseCudaLaunchKernel)
    {
        ...
        if (plist->haveFreshList)
        {
            hipLaunchKernel((void *)nbnxn_kernel_prune_cuda<true>, dim_grid, dim_block, kernel_args, shmem, stream);
        }
        else
        {
            hipLaunchKernel((void *)nbnxn_kernel_prune_cuda<false>, dim_grid, dim_block, kernel_args, shmem, stream);
        }
#endif
    }
    else
