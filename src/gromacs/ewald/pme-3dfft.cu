...
GpuParallel3dFft::GpuParallel3dFft(const PmeGpu *pmeGpu)
{
    ...
    /*
       result = hipfftPlan3d(&planR2C_, realGridSize[XX], realGridSize[YY], realGridSize[ZZ], HIPFFT_R2C);
       handleCufftError(result, "hipfftPlan3d R2C plan failure");

       result = hipfftPlan3d(&planC2R_, realGridSize[XX], realGridSize[YY], realGridSize[ZZ], HIPFFT_C2R);
       handleCufftError(result, "hipfftPlan3d C2R plan failure");
     */
    ...
    GMX_RELEASE_ASSERT(stream, "Using the default CUDA stream for PME hipfft");
...
void GpuParallel3dFft::perform3dFft(gmx_fft_direction dir)
{
    ...
    if (dir == GMX_FFT_REAL_TO_COMPLEX)
    {
        ...
        handleCufftError(result, "hipfft R2C execution failure");
    }
    else
    {
        result = hipfftExecC2R(planC2R_, complexGrid_, realGrid_);
        handleCufftError(result, "hipfft C2R execution failure");
    }
}
